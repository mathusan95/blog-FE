...
export default function LeadForm({ isUpdateForm = false }) {
  ...
  const createModalStatus = useSelector(state => state.crud.createModalStatus);
  const tagsOptionsStatus = useSelector(state => state?.crud?.tagsDataStatus || false);
            ...
            <>
              ...
              {Data.imageComponent.status &&
                <>
                  ...
                  <section className="container">
                    <div>
                      <input type="file" onChange={(file) => handleInput(file, index)} />
              ...
              {Data.codeSnippetComponent.status &&
                <>
                  ...
                  ><b>Enter code </b> </label>:
                  <Editor
                    onValueChange={(value) => handleChangeCode(value, index)}
                    value={components[index].codeSnippetComponent.value || ""}
                    highlight={(codess) => highlight(components[index].codeSnippetComponent.value || "", languages.js)}
                    padding={10}
                    style={{
              ...
              {Data.contentComponent.status && <>
                ...
                {constants.language.map((data) =>
                  <>
                    <label style={{ color: "#e803fc", fontSize: "15px" }}
                    ><b>{`Content in ${data.label}`}</b> </label>:
                    <ReactQuill theme="snow" value={components[index].contentComponent.value[`${data.value}`]} onChange={(val) => handleChangeEditor(val, data.value, index)} />
                    <br />
              ...
              {Data.headingComponent.status &&
                <>
                  ...
                  {constants.language.map((data) =>
                    <>
                      <label style={{ color: "#e803fc", fontSize: "15px" }}
                      ><b>{`Heading in ${data.label}`}</b> </label>:
                      <Input value={components[index].headingComponent.value[`${data.value}`]} onChange={(val) => handleChangeHeading(val, data.value, index)} />
                      <br />
                    </>
                  )}
                </>
              ...
              <br />
              {(components[index].headingComponent.status || components[index].contentComponent.status || components[index].codeSnippetComponent.status || components[index].imageComponent.status) && <Button style={{ backgroundColor: "red", color: "white", float: "left" }} onClick={(e) => addMoreComponent(e, index)}>Remove</Button>}
              {(components[index].headingComponent.status || components[index].contentComponent.status || components[index].codeSnippetComponent.status || components[index].imageComponent.status) && (components.length - 1 == index) && <Button style={{ backgroundColor: "green", color: "white", float: "left" }} onClick={(e) => addMoreComponent(e, index)}>Add More</Button>}
            </>
          )
        })}
      </>
